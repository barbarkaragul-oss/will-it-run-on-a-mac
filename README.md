...
| `emulate ksh` | 9 | `declare -a` |
  ...
  The best of them closes 13 of 47, and **33 of the 47 are closed by nothing**, because they are missing features rather than options: `mapfile`, `shopt`, `${x,,}`, `${x^^}`, `${!name}`, `read -a`/`-p`/`-N`, `compgen`, `complete`, `caller`, `wait -n`, `declare -n`, `type -t`, `printf '%(%Y)T'`, `;;&`. `emulate sh` also has a cost that goes unmentioned: it turns brace expansion off, so `set -- {1..3}` leaves one argument where bash and a plain zsh both leave three. The same answers came from all three runners, which carry the same zsh 5.9.

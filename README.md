...
date -d yesterday
  macOS · BSD       breaks   the binary answered "date: illegal option -- d" (macOS 26.6.2, run on 2026-09-16)
uniq -D hosts.txt
...
Nothing in the flag database is written by hand. [`collector/`](collector/) is a GitHub Actions matrix, run weekly, on **ubuntu-latest** (Ubuntu 24.04, GNU coreutils 9.4), **macos-latest** (macOS 26.6, the BSD userland) and an **alpine:3.20** container (BusyBox 1.36). On each platform it records, for 81 common tools:
...
| [`shells/probe-shells.sh`](collector/shells/probe-shells.sh) | the 143 one-liners in [`shells/probes.txt`](collector/shells/probes.txt) (142 constructs and a version check), each run as a one-line script under every shell the platform has (Ubuntu: bash 5.2, dash as `sh`, dash, zsh, ksh93; macOS: bash 3.2, bash-as-`sh`, zsh, ksh93; Alpine: BusyBox `sh`/ash, bash, zsh, dash), with exit code, first stdout line and first stderr line. zsh is recorded nine times on each platform: plain, under `shwordsplit`, `ksharrays`, `nonomatch`, `octalzeroes`, `bashrematch`, all five at once, `emulate sh` and `emulate ksh`, so the database can answer what a construct does under the configuration the reader has |
...
`probe-all.sh` takes every tool in [`probe-tools.txt`](collector/probe-tools.txt) that the platform has (all 74 on Ubuntu, 71 on macOS, 68 on Alpine), runs each of its short and long flags twice, with and without the operand file, and runs each `find` primary once: 11,696 executions on Ubuntu, 11,304 on macOS and 10,654 on Alpine every week, plus 149, 143 and 137 canary runs. `probe-shells.sh` adds 1,859, 1,716 and 1,859 runs (143 one-liners under 13, 12 and 13 interpreter configurations), and `probe.sh` 87 on each platform. [`scripts/extract.ts`](scripts/extract.ts) turns the recordings into [`data/flags.json`](data/flags.json): per tool and platform, every documented flag with the verbatim line that documents it and its arity, every executed flag with one result (the two runs of a flag become one, *rejected* only if every run was), and the tool's version and path. That is 5,682 flag results for Ubuntu, 5,415 for macOS and 5,244 for Alpine, 16,341 in all. A tool whose canaries printed nothing (`echo`; `zip` on Ubuntu and macOS; `getopt` on macOS; `printf` on Alpine) is executed but keeps no results, because there is no rejection wording to compare its answers with. [`scripts/extract-shells.ts`](scripts/extract-shells.ts) does the same for the shell recordings: [`data/shells.json`](data/shells.json) is 143 probes × 38 interpreter configurations = 5,434 results, 2,002 of them under the 14 shells with their default options and 3,432 under the 24 zsh option settings (eight on each platform). The site loads one small file per tool. The weekly run commits the new database, so a flag that appears in a macOS release shows up as *ok* the next Monday, with the date.
...
- **The classic breaks are still breaks.** `sed -i` without a suffix (macOS takes `'s/foo/bar/'` as the backup extension and then fails on the file name), `sed -z`, `grep -P`, `date -d`, `stat -c`, `find -printf`, `xargs -d`, `du -b`, `cp --reflink`, `cp -u`, `ln -r`, `tar --wildcards`: all rejected on macOS 26, with the binary's own message recorded. `head -n`, `touch -d`, `install -D` and `base64 -w` do exist there, but the Linux way of using them still fails: `head -n -1 f` (a negative count) answers `illegal line count -- -1`, `touch -d '2020-01-01' f` wants `YYYY-MM-DDThh:mm:SS`, `install -D` takes a directory argument (`-D destdir`) so `install -D f d/sub/f2` prints the usage, and `base64 -w0 f` answers `invalid argument f`. `timeout`, `tac` and `nproc` are not there at all.
- **`find` primaries, measured the same way** (86 of them, executed after a path). macOS 26 rejects 15: `-printf`, `-fprintf`, `-fprint`, `-fprint0`, `-fls`, `-regextype`, `-xtype`, `-readable`, `-writable`, `-executable`, `-used`, `-daystart`, `-xautofs`, `-warn`/`-nowarn`; it accepts `-newermt`, `-samefile`, `-uid`/`-gid`, `-wholename`, `-delete`, `-quit`, `-execdir`, `-okdir`. BusyBox rejects 50 of the 86, among them `-newermt` and every other `-newerXY`, `-anewer`/`-cnewer`, `-amin`/`-cmin`, `-iregex`, `-lname`, `-samefile`, `-uid`/`-gid`, `-fstype`, `-true`/`-false`, `-ls`, `-printf`, `-execdir`, `-ok`; it accepts `-delete`, `-print0`, `-regex`, `-empty`, `-quit` and `-wholename`. GNU find 4.9.0 rejects 8: the BSD ones (`-Btime`, `-Bnewer`, `-Bmin`, `-flags`, `-acl`, `-xattr`, `-xattrname`) and `-xautofs`, which no runner accepts; `-newerBt` exists but answers "This system does not provide a way to find the birth time of a file".
- **BusyBox is its own island.** `uniq -D`, `split -d`, `ls -G`, `rm -I`, `base64 -b`, `which -s`, `date -v` are rejected on Alpine; `mktemp -t` exists there but `mktemp -t x`, the macOS way of giving a prefix, answers `Invalid argument`; `date -d` exists there but does not understand `yesterday`.
...
- **zsh is not bash with a different prompt.** Under zsh 5.9 on the macOS runner (the macOS default shell since Catalina; the Linux runners carry the same version), 28 of the 142 constructs that bash 5.2 runs are errors: a glob that matches nothing (`for f in *.tmp` stops with `no matches found`), `[ a == a ]`, `${x,,}`, `${x^^}`, `${!name}`, `${!arr[@]}`, `mapfile`, `read -p`, `read -a`, `read -n`, `read -N`, `shopt` with any option, `declare -n`, `${x@Q}`, `${x@U}`, `$EPOCHSECONDS`, `type -t`, `type -P`, `compgen`, `complete`, `caller`, `wait -n`, `printf '%(%Y)T'`, and `;;&` in a `case`. Nineteen more run under zsh and print something else: `${arr[1]}` is the first element (zsh arrays start at 1), and so are `${arr[$i]}` and the element you read back after `arr+=(x)`; an unquoted `$var` is one word, not three, and `IFS` does not split it either; `echo x | read v` keeps `v` (the last command of a pipe is not a subshell); `echo 'a\tb'` prints a tab where bash prints the backslash; `$((010))` is 10, not 8, and `$((08))` is 8 where bash stops with `value too great for base`; `${PIPESTATUS[@]}`, `$FUNCNAME`, `${BASH_REMATCH[1]}` and `$HOSTNAME` are unset; a `trap ... EXIT` set inside a function fires when the function returns, not when the script ends; `unset arr[1]` leaves three elements; `local` outside a function is allowed; `declare -p x` prints `typeset x=hi`; `declare -F f` prints nothing; `export -f` is accepted and prints the function, as `typeset -f` would. The Linux builds of the same zsh 5.9 reject `export -f` instead; it is the only row where the three zsh runs differ. ShellCheck [does not target zsh](https://github.com/koalaman/shellcheck/issues/809); this is the measured list.
...
- **The compatibility knobs buy less than they are said to.** zsh has options that make it behave more like another shell, and the usual advice is to set them and move on. Measured against bash 5.2, of the 47 constructs a default zsh 5.9 gets wrong:

| what you put at the top of the script | closes | breaks something that worked |
|---|---|---|
| `setopt shwordsplit` | 2 | |
| `setopt ksharrays` | 3 | `declare -a` |
| `setopt nonomatch` | 3 | |
| `setopt octalzeroes` | 2 | |
| `setopt bashrematch` | 1 | |
| all five together | 11 | `declare -a` |
| `emulate sh` | **13** | `{1..3}` brace expansion, `declare -a` |
| `emulate ksh` | 9 | `declare -a` |

  The best of them closes 13 of 47, and **33 of the 47 are closed by nothing**. On the macOS runner 25 of them are errors, features none of the eight recorded settings adds: `mapfile`, `shopt` (four probes), `${x,,}`, `${x^^}`, `${!name}`, `${!arr[@]}`, `${x@Q}`, `${x@U}`, `$EPOCHSECONDS`, `read -a`/`-p`/`-n`/`-N`, `compgen`, `complete`, `caller`, `wait -n`, `declare -n`, `type -t`/`-P`, `printf '%(%Y)T'`, `;;&`. The other 8 run and answer differently, and none of the eight recorded settings changes the answer: `${PIPESTATUS[@]}`, `$FUNCNAME`, `$HOSTNAME`, `declare -p`, `declare -F`, `unset arr[1]`, `local` outside a function, `export -f` (an error on the Linux runners). `emulate sh` also has a cost that goes unmentioned: it turns brace expansion off, so `set -- {1..3}` leaves one argument where bash and a plain zsh both leave three. The same answers came from all three runners, which carry the same zsh 5.9.
...
- **`date +%N` is not an error, it is wrong.** For `date +%s%N`, BusyBox on Alpine exits 0 and prints 10 digits, the seconds alone, where Ubuntu and macOS print 19; exit codes alone would have called it fine. Scenario probes record the first line of output for that reason.
...
The report counts, per construct, how many scripts use it at least once and which ones the probes already measure; a construct many scripts use that no probe covers is a candidate for the next probe. The scanner skips zsh files and only reports how many it skipped, so a repository that is mostly zsh (ohmyzsh) contributes only its `.sh` files.
...
- **Shell constructs are matched by shape, not simulated.** The 142 constructs are recognized in the parse tree; the verdict is the recorded run of that construct's own one-liner under the interpreter, not a run of your line. A construct outside the list is not mentioned. The list is not a specification: it is every construct used by at least 2.5% of 2,905 scripts from 28 of the 30 widely run repositories in [`collector/shells/corpus.txt`](collector/shells/corpus.txt) (counted by [`scripts/corpus-scan.ts`](scripts/corpus-scan.ts) on 2026-09-15; the checkouts of neofetch and thoughtbot/laptop were empty in that scan, so neither is in the count), plus the well-known differences; `<<-` heredocs cannot be a one-liner and are not probed. The three interpreters judged are the ones the shebang reaches (`bash`, `sh`, `zsh`, or the platform default when there is none); ksh, dash-on-Alpine and the eight zsh option settings per platform are recorded but not shown.
...
- **Network, process and host-changing tools** (`curl`, `wget`, `rsync`, `openssl`, `kill`, `pkill`, `hostname`: 7 of the 81) are documented but not executed flag by flag.
